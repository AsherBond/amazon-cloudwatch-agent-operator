...
echo "Enabling Application Signals for EKS Cluster ${CLUSTER_NAME} in ${REGION} with endpoint ${ENDPOINT} for namespace ${NAMESPACE}"
...
eksctl utils associate-iam-oidc-provider --cluster ${CLUSTER_NAME} --region ${REGION} --endpoint ${ENDPOINT} --approve
...
result=$(aws eks describe-addon --addon-name amazon-cloudwatch-observability --cluster-name ${CLUSTER_NAME} --endpoint ${ENDPOINT} --region ${REGION} 2>&1)
echo "${result}"
...
if [[ "${result}" == *"No addon: "* ]];  then
    ...
    status=$(aws eks describe-addon --cluster-name ${CLUSTER_NAME} --addon-name amazon-cloudwatch-observability --region ${REGION} --endpoint ${ENDPOINT}| grep '"status":' | awk -F '"' '{print $4}')
