...
aws eks delete-addon --cluster-name $CLUSTER_NAME --addon-name amazon-cloudwatch-observability --region $REGION --endpoint ${ENDPOINT}

echo "Deleting ServiceAccount"
eksctl delete iamserviceaccount --cluster $CLUSTER_NAME --region $REGION --endpoint ${ENDPOINT} --name cloudwatch-agent --namespace amazon-cloudwatch
